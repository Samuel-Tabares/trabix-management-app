...
export class CrearPedidoStockHandler
    implements ICommandHandler<CrearPedidoStockCommand>
{
    ...
    async execute(command: CrearPedidoStockCommand): Promise<any> {
        ...
        this.logger.log(
            `Pedido creado: ${pedido.id} - ${command.cantidadTrabix} TRABIX`,
        );
        return pedido;
    }
}
...
export class ModificarPedidoStockHandler
    implements ICommandHandler<ModificarPedidoStockCommand>
{
...
export class AgregarCostoPedidoHandler
    implements ICommandHandler<AgregarCostoPedidoCommand>
{
    ...
    async execute(command: AgregarCostoPedidoCommand): Promise<any> {
        ...
        this.logger.log(
            `Costo agregado: ${costo.id} - ${command.concepto}: $${command.costoTotal}`,
        );
        return costo;
    }
}
...
export class EliminarCostoPedidoHandler
    implements ICommandHandler<EliminarCostoPedidoCommand>
{
...
export class ConfirmarPedidoStockHandler
    implements ICommandHandler<ConfirmarPedidoStockCommand>
{
    ...
    async execute(command: ConfirmarPedidoStockCommand): Promise<any> {
        ...
        const insumosObligatorios =
            await this.tipoInsumoRepository.findObligatorios();
...
export class RecibirPedidoStockHandler
    implements ICommandHandler<RecibirPedidoStockCommand>
{
...
export class CancelarPedidoStockHandler
    implements ICommandHandler<CancelarPedidoStockCommand>
{
    ...
    async execute(command: CancelarPedidoStockCommand): Promise<any> {
        ...
        const cancelado = await this.pedidoRepository.cancelar(
            command.pedidoId,
            command.motivo,
        );

        this.logger.log(
            `Pedido cancelado: ${command.pedidoId} - Motivo: ${command.motivo}`,
        );
...
export class CrearTipoInsumoHandler
    implements ICommandHandler<CrearTipoInsumoCommand>
{
    ...
    async execute(command: CrearTipoInsumoCommand): Promise<any> {
        ...
        const existente = await this.tipoInsumoRepository.findByNombre(
            command.nombre,
        );
        if (existente) {
            throw new DomainException(
                'INS_002',
                'Ya existe un tipo de insumo con ese nombre',
            );
        }
        ...
        this.logger.log(
            `Tipo de insumo creado: ${tipoInsumo.id} - ${command.nombre}`,
        );
        return tipoInsumo;
    }
}
...
export class ModificarTipoInsumoHandler
    implements ICommandHandler<ModificarTipoInsumoCommand>
{
...
export class DesactivarTipoInsumoHandler
    implements ICommandHandler<DesactivarTipoInsumoCommand>
{
