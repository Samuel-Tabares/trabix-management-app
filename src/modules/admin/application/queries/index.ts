  ...
  implements IQueryHandler<ListarTiposInsumoQuery, TipoInsumoResponseDto[]>
{
  ...
  async execute(query: ListarTiposInsumoQuery): Promise<TipoInsumoResponseDto[]> {
    const tipos = await this.tipoInsumoRepository.findAll({ activo: query.activo });
...
export class ObtenerStockReservadoHandler implements IQueryHandler<ObtenerStockReservadoQuery> {
    ...
    const [
      ...
      // Ventas de hoy
        this.prisma.venta.aggregate({
            ...
            where: {
                // Se filtra por la fecha en la que la venta fue validada/aprobada
                fechaValidacion: { gte: hoy },

                // Solo ventas en estado APROBADA
                estado: 'APROBADA',
            },
        }),
        // Cuadres pendientes
      ...
      // Vendedores activos
      this.prisma.usuario.count({ where: { rol: 'VENDEDOR', estado: 'ACTIVO' } }),
      // Stock admin
  ...
  implements IQueryHandler<VentasPeriodoQuery, VentasPeriodoDto>
{
  constructor(private readonly prisma: PrismaService) {}
    ...
    async execute(query: VentasPeriodoQuery): Promise<VentasPeriodoDto> {
        // Calcular la fecha de inicio según el período solicitado
        const fechaInicio = this.calcularFechaInicio(query.periodo);

        const resultado = await this.prisma.venta.aggregate({
            // Se usa _all para obtener un conteo numérico consistente
            _count: { _all: true },
            ...
            where: {
                // Se filtra por la fecha de validación/aprobación real del modelo
        ...
        return {
            ...
            // Total de Trabix vendidos en el período
...
export class VendedoresActivosHandler implements IQueryHandler<VendedoresActivosQuery> {
  constructor(private readonly prisma: PrismaService) {}
